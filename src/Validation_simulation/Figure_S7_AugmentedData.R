...
###. FIGURE S7: Augmented Data ###. 
set.seed(123)
for (iS in 1:NB_data.simulated){
  
  ## Reference model
  intraining <- createDataPartition(Y_ref, p = 0.7, list=F)
  rf_train <- randomForest(Y_ref[intraining]~., X_ref[intraining, ], maxnodes = 5, mtry = 25, ntree = 1500)
...
boxplot(AUC_from_ref, AUC_from_BS, AUC_from_mida, 
        col = c("grey", piratepal(palette = "pony")[2:3]), 
        axes=F, ylab = "AUC-ROC")
t.test(AUC_from_BS, AUC_from_mida)
axis(1, at=1:3, labels = c("Ref.", "BiomeSampler", "MidaSim"))
axis(2)
segments(x0 = 2, x1 = 3, y0 = 0.95, xpd = NA)
text(2.5, 0.98, "*", xpd = NA, cex = 2)
