...
n = c(75, 150, 299)
for(N in n){
  res.coda = res.residuals = res.clr = res.rclr = res.PA = res.unifrac = NULL
  res.inter.coda = res.inter.clr = res.inter.rclr = res.inter.uni = res.inter.r = res.inter.PA = NULL
  ...
  for(nb in 1:40){
    ...
    P.A.train <- X.sim[ech.train,]>0 # Presence/absence
    clr.train <- data.frame(clr(coda.train)) # CLR
    rclr.train <- data.frame(vegan::decostand(coda.train, "rclr")) # robust CLR
    d.Unifrac <- picante::unifrac(coda.train, filt_tree) ## Distance UniFrac.
    
    Z_1_R <- cutree(hclust(vegan::vegdist(coda.train), method = "ward.D"), 3) # Clustering
    
    r.train <- data.frame(lm(as.matrix(P.A.train)~as.factor(Z_1_R))$residuals) # Residuals train
    r.pred.train <- data.frame(lm(as.matrix(clr.train)~as.factor(Z_1_R))$residuals) # Residuals train
    
    p.coda = p.r = p.clr = p.rclr = p.PA = p.unifrac = NULL
    spe.coda = spe.r = spe.clr = spe.rclr = spe.PA = spe.unifrac = NULL
    ...
    for(w_1 in c(0, 0.08, 0.15)){
      ...
      # (E) Clustering of rel Ab, clr, rclr, PA and r.
      k_r <- cutree(hclust(dist(r.train), method = "ward.D"), 2)
      k_clr <- cutree(hclust(dist(clr.train), method = "ward.D"), 2)
      k_rclr <- cutree(hclust(dist(rclr.train), method = "ward.D"), 2)
      k_PA <- cutree(hclust(dist(P.A.train), method = "ward.D"), 2)
      k_unifrac <- cutree(hclust(dist(d.Unifrac), method = "ward.D"), 2)
      ...
      p.rclr = c(p.rclr, log(chisq.test(table(data.frame(Y = Y_sim, k = k_rclr)))$p.value))
      # p.value between clustering on PA and Y weigthed.
      ...
      rf <- randomForest::randomForest(as.factor(k_rclr)~.,data.frame(coda.train))
      spe.rclr = c(spe.rclr, mean(as.numeric(log(colMeans(coda.train))[names(sort(rf$importance[,1], decreasing = T))[1:20]])))
  ...
  DF_1 = list(res.coda, res.clr, res.rclr, res.unifrac, res.PA, res.residuals)
  DF_2 = list(res.inter.coda, res.inter.clr, res.inter.rclr, res.inter.uni, res.inter.PA, res.inter.r)
  res = list(DF_1, DF_2)
  
  # save(res, file = paste0("data/figure_3_exp1_", N, ".rda"))
}

for(N in n){
  # load(file = paste0("data/figure_3_exp1_", N, ".rda"))
  pdf(file = paste0("figures/figure_3_exp1_", N, ".pdf"), width = 6, height = 3)
  
  layout(matrix(c(1,2), nrow=1))
  color = LaCroixColoR::lacroix_palette(type = "paired")[c(2, 4, 6, 8, 12)]
  color[6] = LaCroixColoR::lacroix_palette(type = "paired")[11]
  ...
  positions = c(0.8, 0.9, 1, 1.1, 1.2, 1.3)
  for (i in 1:6){
  ...
  ##### FIGURE 2
  plot(1, 1, xlim = c(0.5, 3.5), ylim = c(-4,0), col = 'white', 
...
pdf(file = "figures/legend.pdf", width = 6, height = 3)
plot.new()
legend("bottomleft", legend = c("Rel. Ab.", "CLR", "r-CLR", "Unweighted Unifrac", "PA", "Guided Transf."), fill = color,
