...
df = rbind(paste0(round(apply(res.X$AUC.X, 2, function(x) mean(x[which(x!="NaN")])),2), " (±", round(apply(res.X$AUC.X, 2, function(x) sd(x[which(x!="NaN")])),2),")"),
      ...
      paste0(round(apply(res.rclr$AUC.X, 2, function(x) mean(x[which(x!="NaN")])),2), " (±", round(apply(res.rclr$AUC.X, 2, function(x) sd(x[which(x!="NaN")])),2),")"),
      paste0(round(apply(res.rclr$AUC.R, 2, function(x) mean(x[which(x!="NaN")])),2), " (±", round(apply(res.rclr$AUC.R, 2, function(x) sd(x[which(x!="NaN")])),2),")"),
